<template>
  <header>
    <nav id="mainNavbar" data-sticky="false"
      class="group fixed w-full z-50 top-0 transition-all duration-300 bg-white dark:bg-gray-900 border-b border-gray-200/20 dark:border-gray-700/30">
      <div class="flex flex-wrap justify-between items-center mx-auto max-w-7xl px-4 py-4">
        
        <!-- Logo -->
        <NuxtLink to="/" class="flex items-center group">
          <div class="relative">
            <div class="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform duration-300">
              <span class="text-lg font-bold text-white">TT</span>
            </div>
            <div class="absolute -top-1 -right-1 w-3 h-3 bg-green-400 rounded-full animate-pulse"></div>
          </div>
          <div class="ml-3">
            <span class="text-xl font-bold text-gray-900 dark:text-white transition-colors">
              TorkataTech
            </span>
        ...
        <div class="hidden lg:flex items-center space-x-1">
          <NuxtLink to="/"
            class="px-4 py-2 text-gray-700 dark:text-gray-200 hover:text-blue-600 transition-colors duration-300 font-medium rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800">
            Home
          </NuxtLink>
          ...
          <div class="relative" @mouseenter="handleDropdownEnter" @mouseleave="handleDropdownLeave">
            <button
              class="flex items-center px-4 py-2 text-gray-700 dark:text-gray-200 hover:text-blue-600 transition-colors duration-300 font-medium rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800">
              Blog
              <svg class="w-4 h-4 ml-1 transition-transform duration-300" :class="{ 'rotate-180': showBlogDropdown }" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div v-show="showBlogDropdown" 
              class="absolute top-full left-0 mt-2 w-64 bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden transform transition-all duration-300"
              ...
              <div class="p-2">
                <NuxtLink to="/blog" 
                  ...
                  <div class="w-8 h-8 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex items-center justify-center mr-3 group-hover:bg-blue-200 dark:group-hover:bg-blue-800/50 transition-colors">
                ...
                <template v-if="data?.data?.length">
                  <NuxtLink v-for="category in data.data" :key="category.id" :to="`/blog/category/${category.slug}`" 
                    ...
                    <div class="w-8 h-8 bg-purple-100 dark:bg-purple-900/30 rounded-lg flex items-center justify-center mr-3 group-hover:bg-purple-200 dark:group-hover:bg-purple-800/50 transition-colors">
        ...
        <div class="flex items-center space-x-3">
          ...
          <button id="theme-toggle" type="button"
            class="p-2.5 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500/50">
          ...
          <button id="mobile-menu-button" @click="toggleMobileMenu" type="button"
            class="lg:hidden p-2.5 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition-all duration-300 focus:outline-none">
      ...
      <div v-show="showMobileMenu" 
        class="lg:hidden bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 transition-all duration-300"
        ...
        <div class="px-4 py-6 space-y-2">
          ...
          <div>
            <button @click="toggleMobileBlog" 
              ...
              Blog
              <svg class="w-4 h-4 transition-transform duration-300" :class="{ 'rotate-180': showMobileBlog }" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"/>
              </svg>
            </button>
            <div v-show="showMobileBlog" class="ml-4 mt-2 space-y-2">
              ...
              <template v-if="data?.data?.length">
                <NuxtLink v-for="category in data.data" :key="category.id" :to="`/blog/category/${category.slug}`" @click="closeMobileMenu"
                  class="block px-4 py-2 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors">
                  {{ category.name }}
                </NuxtLink>
              </template>
